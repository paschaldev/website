...
You may find that certain shades that have less common names pair better with your main accent color. For example, it is common to use `crimson` and `ruby` in place of `red`, or `teal` and `jade` in place of `green`.

To reclaim the more intuitive color names like `red`, `green`, or `blue`, among others, you can remap Radix Themes tokens in place of one another:
