...
const StyledIndicator = styled(NavigationMenu.Link, {
...
If you need to use a `Link` provided by your routing package then we recommend composing with `NavigationMenu.Link` via a custom component.
This will ensure accessibility and consistent keyboard control is maintained. Here's an example using Stitches and React Router:

```jsx line=14-26,32,35
import {
  useMatch,
  useResolvedPath,
  Link as RouterLink,
...
const CustomLink = ({ children, to, ...props }) => {
  ...
  return (
    <StyledLink active={isActive} asChild>
...
See [this example](/docs/primitives/components/navigation-menu#with-router-links) for more information on usage with third party routing components.
